...
* <<mqadminapi,MQ Admin API>> - Get an access token for an Anypoint Platform user.
* <<mqbrokerapi,MQ Broker API>> - Get a token from Anypoint MQ by supplying the client ID and client secret for an MQ application.
* <<mqstatapi,MQ Stats API>> - Get an access token for an Anypoint Platform user.
...
*Note:* The organization from which you use the Administration portal must have an Anypoint MQ FIFO entitlement. See xref:mq-faq.adoc[MQ FAQ] for the regions MQ is available in.
...
----
+
See the xref:mq-faq.adoc[MQ FAQ] for the regions that MQ and FIFO are available in.
+
...
* View historical data for queue activity over time. 
* View your entire organization's usage of MQ for billing purposes.

The Anypoint MQ Statistics API provides access to Anypoint MQ message queues.
...
After you set up an HTTP Listener in Studio, browse to this address: +0.0.0.0:8081+. The browser displays the message sent by Postman, which the Anypoint MQ connector received, and the HTTP connector sent to the browser.
